...
parser.add_argument('--checkpoint-path', default='checkpoints/ampli_Rayleigh', type=str)
...
parser.add_argument('--ibo-db', type=float, default=None, help='Input Back-Off (dB) for amplifier saturation')
    ...
    with torch.no_grad():
        for sents in pbar:
            ...
            loss = val_step(net, sents, sents, 0.1, pad_idx,
                             criterion, args.channel, ibo_db=args.ibo_db)
...
def train(epoch, args, net, mi_net=None):
    ...
    for sents in pbar:
        ...
        if mi_net is not None:
            mi = train_mi_step(net, mi_net, sents, noise_std, pad_idx, mi_opt, args.channel, ibo_db=args.ibo_db)

        if mi_net is not None:
            loss_total, loss_ce, loss_mi = train_step(
                net, sents, sents, noise_std, pad_idx,
                optimizer, criterion, args.channel, mi_net, ibo_db=args.ibo_db
            )
        else:
            loss_ce = train_step(net, sents, sents, noise_std, pad_idx,
                                 optimizer, criterion, args.channel, ibo_db=args.ibo_db)
            loss_total = loss_ce
            loss_mi = 0
